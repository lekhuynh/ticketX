...
class ShowtimeService:
    ...
    async def create_showtime(self, showtime_data: ShowtimeCreate) -> Showtime:
        ...
        event = await self.event_repo.get_by_id(showtime_data.event_id)
        if not event:
            raise HTTPException(
        ...
        await refresh_ai_docs_for_event(self.session, str(created_showtime.event_id))
    ...
    async def update_showtime(self, showtime_id: uuid.UUID, showtime_data: ShowtimeUpdate) -> Showtime:
        ...
        await refresh_ai_docs_for_event(self.session, str(updated_showtime.event_id))
    ...
    async def delete_showtime(self, showtime_id: uuid.UUID) -> None:
        ...
        await self.session.commit()
        await refresh_ai_docs_for_event(self.session, str(showtime.event_id))
