...
class EventService:
    ...
    async def create_event(self, venue_id: uuid.UUID, event_data: EventCreate) -> Event:
        ...
        await refresh_ai_docs_for_event(self.session, str(result.id))
        return result
    ...
    async def delete_event(self, event_id: uuid.UUID) -> None:
        delete_event_obj = await self.get_event(event_id)
        await self.repo_event.delete_event(delete_event_obj)
        await self.session.commit()
        await refresh_ai_docs_for_event(self.session, str(event_id))